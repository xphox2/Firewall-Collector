...
// a FreeBSD shell. An empty vendor defaults to FortiGate (legacy behavior, same
// as the SNMP vendor resolver). Unknown vendors return an error so the caller
// logs-and-skips rather than mis-driving a device with the wrong CLI dialect.
