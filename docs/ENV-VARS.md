...
| Variable | Default | Description |
|---|---|---|
| `PROBE_STRICT_SOURCE_BINDING` | `true` | Guards the two config/telemetry paths that attribute by a body/filename field rather than the packet source, so an allowlisted fleet device can't submit data in another monitored device's name. **sFlow** (attributes by the in-band `agent_address`): a sample is **rejected** when the UDP source resolves to a *uniquely-attributable* monitored device that disagrees with the claimed device (a source/claim mismatch). It is **accepted** when the source can't be uniquely resolved — an uncached multi-homed egress IP, or an **HA/CARP shared VIP or A-P cluster synced interface** the collector has seen from two members (those are recorded with the set of devices sharing them, so a member of the set is accepted while an outsider forging the shared VIP is still rejected). **TFTP** config backup (attributes by the WRQ filename): the collector initiates every backup and composes its filename, so an upload is **accepted** whenever the claimed device has a live collector-issued backup trigger — *regardless of the WRQ source IP*, which keeps a branch device SNATing through a monitored NAT/hub working — and is treated as unsolicited (a forged cross-device claim, or a stale upload) only when the device has zero live triggers. The trigger registry is count-valued, so a benign double-trigger (the poll and syslog backup paths both firing for one device within the window) admits both legitimate uploads. In STRICT (`true`) a mismatch/unsolicited upload is dropped; in warn-only (`false`) it is logged and still attributed by claim/filename. Accepts the case-insensitive `true`/`false`/`1`/`0`/`yes`/`no`/`on`/`off` forms. Rejects are counted on `firewall_collector_source_binding_rejects_total` (labeled by path `sflow`/`tftp`) — a nonzero value is more often a topology/config issue (an undeclared shared interface, a NAT not fronted by a monitored device) than an attack. Layers on top of, and does not replace, the fleet source-IP allowlist. |
