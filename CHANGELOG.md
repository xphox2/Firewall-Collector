...
- **sFlow source binding (AUDIT-186):** sFlow attributes a sample by the in-band `agent_address` (the datagram body), while the existing spoofing guard only checks the UDP source — so a datagram from any allowlisted source could carry any `agent_address` and be attributed to a different monitored device (forging its flow/counter graphs and gaming the dedup key to suppress the victim's real flows). The real UDP source is now threaded onto every flow and counter sample and bound to the claimed identity before attribution and before the dedup gate. A sample is rejected only when the UDP source resolves to a *uniquely-attributable* device that disagrees with the claim; a source that can't be uniquely resolved is accepted (attributed by claim).
- **HA/CARP-safe resolution, device-set scoped (AUDIT-186):** interface IPs the collector has seen from two different devices — OPNsense CARP VIPs, FortiGate A-P cluster synced interfaces, or a live IP-reassignment window — are recorded with the *set of devices that share them*, so a healthy HA member exporting sFlow with a shared cluster IP as its `agent_address` is never false-rejected (which last-writer-wins resolution would otherwise do about half the time, flapping with poll order). The set still tightens the guard: a sample whose source is a *member* of the sharing set is accepted, but a KNOWN source that is NOT a member — an outsider forging the cluster's identity — is still rejected. Per-device-unique management IPs resolve authoritatively, so a genuine cross-device forgery on unique addresses also still rejects.
- **TFTP expectation binding, count-valued (AUDIT-187):** the TFTP config-backup device ID was derived entirely from the attacker-writable WRQ filename (`fgt_<id>_<trigger>_config`) with no source check — so an allowlisted device could upload `fgt_<victimID>_..._config` and forge an authoritative config revision for another device. Because the collector initiates every backup and composes the filename itself, uploads are now bound to a short-lived collector-issued pending-trigger registry: an upload is recorded when the claimed device has a live trigger — *regardless of the WRQ source IP*, so a branch device SNATing through a monitored NAT/hub is recorded correctly — and an upload with zero live triggers (an unsolicited/forged cross-device claim, or a stale upload past the window) records no `ConfigRevision`. The registry counts triggers per device, so a benign double-trigger (the poll and syslog backup paths both firing for one device within the window) admits both legitimate uploads instead of consume-rejecting the second and mislabeling it a forgery. This keeps the forgery guard (an attacker can't get an unsolicited upload recorded for a victim) while being NAT-proof.
- **`PROBE_STRICT_SOURCE_BINDING` toggle (default `true`):** STRICT (reject a source/claim mismatch and an unsolicited upload) is the default; an operator can set it to `off`/`false` to downgrade to warn-only (log the mismatch but still attribute by claim/filename) if a deployment surfaces a false positive. Parsed with the case-insensitive on/off/true/false/1/0/yes/no forms and documented in `docs/ENV-VARS.md`.
- **Observability:** rejects are counted on a new Prometheus counter `firewall_collector_source_binding_rejects_total` (labeled by path `sflow`/`tftp`); a sustained nonzero value is more often a topology/config issue (an undeclared shared interface, a NAT not fronted by a monitored device) than an attack.
...
- **TFTP shutdown & read-path guards (AUDIT-306, AUDIT-307):** the WRQ write-handler callback (config-revision send) is now tracked on the server WaitGroup, so Shutdown blocks until an in-flight revision finishes instead of abandoning it. The RRQ read path gained the same source-IP allowlist and per-source rate-limit guards the WRQ path already had, with the per-packet "RRQ from" log moved to after those checks.
- **NetFlow/IPFIX parser limits (AUDIT-282, AUDIT-283, AUDIT-284):** the IPFIX field-spec parser now bounds the wire-declared field count by the bytes actually present before allocating, so a 0xFFFF field count in a tiny datagram can no longer force a ~512 KB allocation. The sequence-loss tracker gained an idle-eviction sweep wired into the maintenance ticker, so a sender enumerating the domain field can no longer pin the state map at its cap and silently disable gap detection. A NetFlow v9 template carrying a 0xFFFF (variable-length) field — which does not exist in v9 — is now quarantined instead of desyncing every record for that exporter.
- **Log-flood caps (AUDIT-216, AUDIT-305):** an unrecognized SNMP trap now logs a bounded number of varbind lines plus a "+N more" summary instead of one line per varbind. The UDP syslog path now counts malformed datagrams instead of logging one line per datagram (matching the TCP path), and both syslog receivers surface that count through a new Prometheus counter `firewall_collector_syslog_parse_errors_total` (labeled by transport `udp`/`tcp`) — so removing the per-message log does not leave the malformed-syslog signal invisible to operators.
- **Interface-IP cache pruning (AUDIT-237):** the interface-IP→device-ID resolution cache (and the sibling per-device caches) are pruned on each device-list refresh, so a decommissioned device's IP reassigned to a new device no longer resolves to the stale device until restart.
- **Config boolean parsing (AUDIT-263):** parseBool is now case-insensitive, accepts true/false/1/0/yes/no/on/off, and falls back to the caller's default (with a warning) on an unrecognized value instead of silently reading it as false — so a YAML-capitalized `True` no longer disables a listener that defaults on, and `off` correctly disables an enabled-by-default listener rather than being treated as an unrecognized value.
- **Constant-time trap community compare (AUDIT-317):** the SNMP-trap community check now uses crypto/subtle constant-time comparison, matching the server twin; empty-community-means-open behavior is preserved.
...
### Fixed
- **SD-WAN health metrics were ALL silently zero/garbage (AUDIT-177, High).** The fgVWLHealthCheckLinkTable column OIDs omitted the table-entry level (`.2.1`), so the "name" constant was the table node itself and its prefix branch swallowed every walked PDU: a dead SD-WAN link reported 0% packet loss with an empty state, indistinguishable from a healthy one. All seven column OIDs are corrected, and rows are now keyed by the full index suffix so a composite-indexed table (health-check id + member id) can no longer silently merge distinct links into one row. An SD-WAN state value outside the assumed alive(0)/dead(1) enum is logged once per value (and conservatively mapped to dead), so a device speaking a different enum surfaces immediately instead of misreporting silently.
- **SD-WAN packet loss now comes from the device's own computed column (AUDIT-219).** The parser reads fgVWLHealthCheckLinkPacketLoss directly, and that value is authoritative whenever the column is present — including a healthy link's 0. The old send/recv subtraction runs only when the column is absent from the walk (send/recv are cumulative counters, so on a link that recovered from a past outage the subtraction would report phantom loss forever) and is underflow-guarded — counter timing skew where received exceeds sent previously produced a loss figure around 18 quintillion percent.
...
### Fixed
- **A server outage no longer shreds the disk spool during the drain (AUDIT-175, High).** When the central server was unreachable, one `syncData` cycle collapsed an arbitrarily large multi-day disk spool to roughly 1,000 items within minutes: the drain loop pulled the whole backlog out of the spillover queue in chunks, and on the first transient send failure it requeued only the failed ~1,000-item chunk while silently dropping everything else it had already drained — exactly the outage the queue exists to survive. Now the failed chunk AND the entire unattempted tail are requeued to the queue HEAD (a new bounded `PushFront` front tier in the spillover queue) and the drain stops until the next sync. Head-requeue also means the next drain re-chunks from index 0, so a full-size failed chunk replays byte-for-byte. The drain additionally commits the head tier's removal only after the disk transaction succeeds, so a bolt write failure under disk pressure cannot destroy the requeued items.
- **Requeued batches replay under their original idempotency key (AUDIT-213/214).** The old requeue pushed failed items back one-at-a-time onto the queue TAIL, so the next sync regrouped them with newer items into differently-keyed batches. A batch the server had already committed — but whose response timed out — therefore replayed under a key the server's `(probe_id, batch_id)` dedup could not match, inserting every row twice (duplicated cumulative interface counters then skew the delta-based bandwidth math). The head-requeue above sharply narrows the duplicate window with zero server changes: a full-size failed chunk re-chunks from index 0 on the next drain and replays byte-identically, so the server dedups it. Narrowed, not closed — a partial final chunk that gains newly-arrived neighbors on the next drain, a requeue split by the in-memory bound (the overflow falls back to the tail), and a process restart (the held run is flushed behind the pre-existing disk tier) can each still regroup that one batch under a fresh key: duplicates remain possible in those cases, loss never.
- **A panicking command handler no longer crash-loops the collector (AUDIT-212).** Heartbeat-delivered server commands (including IPSec deploys) ran on a bare `go` statement with no recovery, so one panicking handler — a malformed payload hitting an unguarded code path — killed the whole collector process; the server then redelivered the same command on the next heartbeat after restart, up to 5 attempts: a process-killing crash loop that also took down all SNMP/syslog/sFlow collection. The dispatch goroutine now runs under `safego.Go`, each handler invocation is individually recovered so the panic becomes an ordinary `failed` result the server consumes on attempt 1 (ending redelivery), the rest of the batch still executes, and the in-flight dedup entry is cleared via `defer` so the command can never wedge as forever-in-flight. The stack is logged locally only — neither the stack nor the command payload (which may carry credentials) ever enters the server-stored result.
- **A 404 on `/flow-counters` no longer silently disables half the collector (AUDIT-288).** One 404 on that single endpoint — a proxy blip or a mid-rollout server was enough — collapsed the negotiated schema version to v1, which also switched off the unrelated disk/load telemetry, the entire command channel (IPSec deploys included), and topology collection until the collector was restarted. The response is now scoped to the endpoint that produced the evidence: `/flow-counters` alone is suppressed for a 30-minute self-expiring window (also cleared immediately by a successful re-registration), and every other schema-gated feature keeps running.
- **Oversized batches are dropped instead of retried forever (AUDIT-287).** HTTP 413/414 were treated as transient, but the retry replays a byte-identical body — too large once is too large every time — so an oversized batch was requeued and re-sent every sync cycle indefinitely, evicting good telemetry at the disk byte cap. Both statuses are now permanent rejections across every send path, and the drop log names the operator remedy: the collector already clamps to the server's 1,000-item cap, so a 413 means a proxy in front of the server caps request bodies below the server's limit — lower `PROBE_MAX_BATCH_SIZE` or raise the proxy's `client_max_body_size`.
- **Six unsynchronized probe-ID reads fixed (AUDIT-289).** The config-revision, process-snapshot, interface-error (both), sensor-detail, and license-detail senders built their URL from a bare `c.probeID` read that races the mutex-guarded write during re-registration — a data race the Go race detector flags. All six now use the mutex-guarded accessor; no lock is held across HTTP calls.
...
- Adversarial review fixes (pre-merge): `walkQuiet` no longer swallows non-timeout errors — gosnmp already terminates unsupported subtrees cleanly (nil error), so the swallow only ever ate REAL failures (v3 auth/crypto, max-retries, OID-not-increasing) and could mark a transiently-broken device's ARP table "affirmatively empty", triggering the SSH supplement and the exact SSH/SNMP alternate-overwrite the design forbids (test pins walk-error → flag stays false). Topology walks (and the SSH ARP supplement) are now skipped entirely below a negotiated schema v5 — no more full-table walks whose results the send gate would discard against a pre-0.11.94 server. CDP neighbor management IP moved to a proper `remote_mgmt_addr` field (was smuggled in `remote_caps`; wire still unshipped). `snmpARPEmpty` flags pruned on device-list refresh; SSH ARP truncation now logged; multi-VDOM FortiGate limitation documented (`get system arp` needs a VDOM context — supplement yields nothing there).
...
- **OPNsense devices now report hardware sensors** (temperature, per-rail voltage/current/power, fan speed) that are invisible to SNMP (net-snmp exposes no LM-SENSORS MIB). The SSH poll reads them in one `sysctl -iq hw.temperature dev.ina2xx dev.emc2302` and relays them through the existing `HardwareSensor` pipeline (server-side unchanged). Live-validated on an NXP Layerscape appliance: 34 sensors — 8 temps, 8 PSU rails ×(V/A/W), 2 fans.
  ...
  - Robustness: `sysctl -i` ignores unknown OIDs so a partial-tree or x86 OPNsense (which has none of these drivers) degrades to fewer/zero sensors instead of discarding the whole read on a non-zero exit; unlabeled rails and `%`/`shunt_voltage`/`pwm` noise lines are skipped; sensors are stamped with a single client-side timestamp so the server's content-hash batch dedup can't silently drop an idle-appliance poll. Fans are keyed by controller-unit **and** channel (a >2-fan appliance has multiple EMC230x controllers whose `fan0`s would otherwise collide) and numbered sequentially; a fan reporting only a fault (no rpm) still surfaces as an alarm rather than being dropped.
- Tests: full-fixture parse (counts + native units + fan-fault), x86/partial/empty/garbage degradation, unlabeled-rail skip, multi-controller fan numbering, fault-without-rpm.
...
- **v9/IPFIX template engine** with the traps kept apart: **two framing loops** (IPFIX trusts the header Length; v9 walks FlowSet lengths and ignores the ambiguous Count) and **separate options-template parsers** (v9 uses BYTE lengths, IPFIX uses FIELD counts — sharing one parse path silently corrupts the sampler table). Templates keyed by (exporter IP, sourceID/ODID, templateID) so multi-VDOM/multi-context exporters can't cross-corrupt; IPFIX varlen fields (255-escape) and enterprise-bit + PEN fields decoded; RFC 6313 structured data and withdrawal records parsed-then-ignored; redefinition is an atomic in-place replace (RFC 7011 §8.4); a sanity quarantine rejects absurd field/record lengths (the FortiGate length-9 69× bug class) with a metric; TTL 30 min + caps (256/exporter, 8192 total); **template + sampler caches persist to `<PROBE_QUEUE_DISK_PATH>/netflow-templates.json`** (shutdown + every 5 min) so a restart doesn't blind decoding for a template-refresh cycle.
- **Counter mapping — three families with per-record fallback**: IN_BYTES/IN_PKTS (1/2, any 1–8-byte encoding) → OUT_BYTES/OUT_PKTS (23/24) → initiatorOctets/responderOctets (231/232 — **Cisco ASA NSEL's only byte fields; packets stay 0 because ASA sends none, and zero-counter create/denied records are legal rows** — denied-flow visibility is the headline NetFlow win). IE 85/86 running totals are never summed as deltas (skip + metric). **Biflow**: reverse counters from any of the three encodings (PEN-29305 reverse IEs / FortiGate 23-24-alongside-1-2 / responderOctets) normalize to one reverse-row emission with the 5-tuple mirrored, only when non-zero; reverse-only records are dropped.
- **Sampler resolution chain** (nfdump-style): operator override per exporter IP (`SetSamplingOverride` — the MikroTik-ROS6 escape hatch) → per-sampler-ID from options data (48/302) → domain rate from options (305 [+306], 50, 34 — **including IE 34 arriving inside data records**, MikroTik) → 1. Flows are never held waiting for options. **Counters are pre-multiplied by the resolved rate at the collector — including FortiGate 7.6 sampled NetFlow, whose own doc's worked example proves it exports sampled-scale counters** (the research's headline correction).
- **Timestamp resolution** (`resolveFlowTimes`): 152/153 ms > 150/151 s > 154–157 64-bit NTP format (era-disambiguated, µs fraction bits masked) > 21/22 uptime math (v9 only, wrap-safe unsigned subtraction — the PAN/FortiGate hot path) > 158/159 delta-µs > 323 (+161 duration). Everything plausibility-clamped ±15 min against receive time (softflowd/Catalyst/Nexus clock breakage), preserving the flow's internal duration when believable. `FlowStart`/`FlowEnd` populated on every row; legacy `Timestamp` = flow end.
- **Sequence-loss tracker** keyed (exporter, domain, version) with version-specific deltas (v5 +records / v9 +1 per packet / IPFIX +data records), reboot-aware resync — surfaced as `seq_gap`/`seq_resync` parse events, **never** `FlowSample.Drops` (which keeps its sFlow agent-drop semantics; NetFlow rows always carry 0).
- **New `internal/flowdedup` tracker + `PROBE_FLOW_DEDUP`** (`prefer-netflow` default / `prefer-sflow` / `off`): when one device exports BOTH sFlow and NetFlow, the non-preferred family's FLOW samples are suppressed per exporter while the preferred family is live (5-minute activity window, automatic failover back). sFlow COUNTER samples always pass — interface counters aren't flow records. Suppression state changes are logged once per (exporter, family), not per packet; suppressed samples count in a new metric.
- **Wire contract**: `relay.FlowSample` gains `flow_source`, `flow_start`/`flow_end`, `firewall_event`, `flow_end_reason`, the post-NAT tuple, `icmp_type_code`, `tos`, `src_vlan`/`dst_vlan`, and `app_name` (PAN App-ID 56701 exporter truth) — all `omitempty`, no `schema_version` bump; old servers ignore the unknown keys.
- **Observability**: `firewall_collector_netflow_events_total{event}` (malformed, data_before_template, template_quarantined, template caps, total-counter skips, reverse-only drops, withdrawals, seq_gap/seq_resync, unknown_version), `firewall_collector_flowdedup_suppressed_total{family}`, rate-limit/allowlist drops under the existing `rate_limited_drops_total` (labels `netflow`, `netflow_srcip`), `listener_bound{listener="netflow"}` + `/readyz` registration — only when enabled with ≥1 port; enabled-with-both-ports-0 warns and is skipped instead of wedging readiness at 503.
- **New `cmd/netflow-test` sender** for operators and E2E: crafted spec-correct datagrams for every conformance shape — RFC-clean v5/v9/IPFIX (incl. an IPFIX varlen field), FortiGate (sampler options + sampled-scale counters + OUT_BYTES biflow), PAN (raw 56701 App-ID + uptime timestamps), ASA NSEL (231/232-only teardowns + a zero-counter denied record with `firewall_event=3`), SonicWall (PEN-8741 enterprise field interleaved), MikroTik (IE 34 in data records). Flags: `-target`, `-proto`, `-rate`, `-bytes`, `-count`; prints each packet sent and the expected server-side outcome.
- **Tests**: v5 wrap/clamp/count-lie fixtures; v9-bytes vs IPFIX-counts options parsing (the corruption trap); varlen incl. the 255-escape; enterprise/PEN fields; withdrawal records; template quarantine + caps + TTL; persistence round-trip (restart without the 30-min blind window); the three counter families + biflow normalization; FortiGate sampled-scale multiplication; ASA byte-only mapping (bytes land, packets 0) + zero-byte denied acceptance; NTP-format era edges; uptime wrap; sampler-chain precedence incl. IE-34-in-data; sequence wraparound + reboot resync; dedup activation/failover; allowlist/rate-limit gate order; `FuzzParseNetFlowDatagram` (inputs fed twice — templates are stateful).

### Fixed
- **sFlow emission gate no longer relays address-less garbage rows** (research §3.6): the old `|| seqNum > 0` clause was effectively always true (every sample has a sequence number), so agents sending record types we don't parse produced phantom `0.0.0.0→0.0.0.0` flow rows that poisoned analytics. The gate is now src/dst-address-only.
- **Corrected the wrong "per-sample delta" comment on `FlowSample.Drops`**: the sFlow v5 spec defines it as the CUMULATIVE drop count since agent start. The code was already right (the server baselines first sight and records increases as deltas); only the comment lied.
- sFlow's `sub_agent_id` read is now documented as the observation-domain analogue for future sFlow-side sequence-gap tracking.

### Docs
- `docs/ENV-VARS.md` (new NetFlow/IPFIX ports, toggle, rate limits, `PROBE_FLOW_DEDUP`, template-cache persistence note), `docs/FEATURES.md` (NetFlow row → Stable [Both] 1.3.0 with vendor coverage; new hardening/tooling rows), `docs/ARCHITECTURE.md` (listener diagram, package map, new "Flow pipeline" section), `README.md` (feature bullet, port tables, env table).
...
### Fixed
- **A malformed sFlow record can no longer bleed adjacent bytes into flow/counter telemetry (audit 2026-07-01 finding L9).** The flow-record and counter-record dispatch loops handed each sub-parser the whole datagram with a shared offset, and the `readUint32`/`readUint64` helpers bound their reads against `len(datagram)` — not the current record's end. A record that declared a length shorter than the fields it actually parsed (`parseRawPacketHeader`, `parseExtendedGateway`, `parseIfCounters`) therefore read straight past `recEnd` into the following record, folding its bytes into `SrcAS`/`ASPath`/next-hop or the 64-bit interface octet counters — a single crafted packet could persist a garbage AS path or a fake multi-exabyte counter spike (no crash, so it went unnoticed). Each record is now parsed from a slice bounded to its own declared length (`data[off:recEnd]`) with a record-local offset, so a lying `recLen` can never reach beyond the record. Regression test `TestParseSFlowDatagram_LyingRecLenNoBleed_L9`.
- **Malformed syslog priorities are now rejected instead of being classified as critical (audit 2026-07-01 finding L10).** `parsePriority` located the closing `>` loosely (falling back to end-of-field when absent) and ran the enclosed bytes through `bytesToInt`, which silently dropped non-digit characters — so `<abc>` and `<>` decoded to priority 0 (severity 0 = Emergency), and a long digit run could overflow `int` into a negative value that slipped past the `>191` upper-bound check into a negative severity. Both cases made the server treat garbage as critical and retain it for 30 days. Parsing now enforces the RFC 5424 PRIVAL grammar: a leading `<`, a closing `>`, and 1–3 all-digit bytes encoding 0–191; anything else is an error.
- **A persistent UDP read error no longer blackholes sFlow agents or spins the syslog receiver (audit 2026-07-01 finding L11).** The sFlow `readLoop` returned on the first non-timeout read error, leaving its `SO_REUSEPORT` socket bound so the kernel kept hashing 1/N of the agents to a dead fd until a process restart (all of them with a single worker); the syslog receiver took the opposite tack and `continue`d, spinning a tight hot-loop that re-read the errored socket. Both receivers now supervise each worker socket: on a persistent read error they close the dead fd (the kernel drops it from the reuseport group and rebalances to the live workers) and reopen a fresh listener after a growing backoff (200 ms → 30 s). `Stop()` snapshots the connection slice under a new `connsMu` mutex so it closes whichever fd a worker currently holds after a respawn.
