...
- **HA/CARP-safe resolution (AUDIT-186):** interface IPs the collector has seen from two different devices — OPNsense CARP VIPs, FortiGate A-P cluster synced interfaces, or a live IP-reassignment window — are detected as ambiguous and treated as unresolvable by the binding, so a healthy HA member exporting sFlow with a shared cluster IP as its `agent_address` is never false-rejected (which last-writer-wins resolution would otherwise do about half the time, flapping with poll order). Per-device-unique management IPs still resolve authoritatively, so a genuine cross-device forgery on unique addresses still rejects.
- **TFTP expectation binding (AUDIT-187):** the TFTP config-backup device ID was derived entirely from the attacker-writable WRQ filename (`fgt_<id>_<trigger>_config`) with no source check — so an allowlisted device could upload `fgt_<victimID>_..._config` and forge an authoritative config revision for another device. Because the collector initiates every backup and composes the filename itself, uploads are now bound to a short-lived collector-issued pending-trigger registry: an upload is recorded when the claimed device has a live trigger — *regardless of the WRQ source IP*, so a branch device SNATing through a monitored NAT/hub is recorded correctly — and an upload with no pending trigger (an unsolicited/forged cross-device claim, or a stale upload past the window) records no `ConfigRevision`. This keeps the forgery guard (an attacker can't get an unsolicited upload recorded for a victim) while being NAT-proof.
