...
- **Log-flood caps (AUDIT-216, AUDIT-305):** an unrecognized SNMP trap now logs a bounded number of varbind lines plus a "+N more" summary instead of one line per varbind; the UDP syslog path now counts malformed datagrams via a metric instead of logging one line per datagram (matching the TCP path).
...
- **Config boolean parsing (AUDIT-263):** parseBool is now case-insensitive, accepts true/false/1/0/yes/no, and falls back to the caller's default (with a warning) on an unrecognized value instead of silently reading it as false — so a YAML-capitalized `True` no longer disables a listener that defaults on.
